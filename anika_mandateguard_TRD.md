...
| Policy spec | YAML/JSON | Source of truth for gold-label generation and for the gateway's rules |
| Tool layer | 5 tools: `get_product`, `present_offer`, `request_approval`, `create_checkout`, `verify_payment` | Thin wrappers; `create_checkout`/`verify_payment` call Razorpay test-mode APIs |
...
- **Deterministic (MandateGuard):** budget math, per-item cap, merchant/category matching, expiry check, duplicate/idempotency detection, price/inventory freshness comparison.
...
- **Hard rule:** no LLM output is ever treated as ground truth for whether a transaction complies with the mandate.
...
- On timeout, the agent's retry reuses the same key; MandateGuard rejects a second execution with the same key and returns the original result.
...
**Gold-label methodology:** labels are derived programmatically from the policy spec (not hand-judged) — write the YAML rule first, generate the scenario to exercise it, and derive the expected allow/block/approve outcome directly from the rule.
...
- Duplicate webhook / stale payment-success handling: `verify_payment` re-checks against the idempotency key before marking a mandate as fulfilled.
